...
class ReRanker:
    """Combines semantic, recency, and relational signals."""

    def __init__(self, w_sem: float = 0.6, w_rec: float = 0.2, w_rel: float = 0.2):
        self.w_sem = w_sem
        self.w_rec = w_rec
        self.w_rel = w_rel
    ...
    def update_weights(self, intent: str) -> None:
        if intent == "relational":
            self.w_rel, self.w_sem, self.w_rec = 0.4, 0.4, 0.2
        elif intent == "follow_up":
            self.w_rec, self.w_sem, self.w_rel = 0.5, 0.4, 0.1
        else:
            self.w_sem, self.w_rec, self.w_rel = 0.6, 0.2, 0.2
    ...
    def score(
        ...
        return (self.w_sem * sem) + (self.w_rec * rec) + (self.w_rel * rel) 